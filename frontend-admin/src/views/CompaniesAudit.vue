<template>
  <div>
    ...
    <el-table :data="list" border>
      ...
      <el-table-column prop="industry" label="行业code" width="100" />
      <el-table-column prop="licenseNo" label="执照号" width="200" />
      ...
      <el-table-column label="操作" width="160">
        <template #default="{ row }">
          ...
          <span v-else-if="row.rejectReason" class="reason">理由：{{ row.rejectReason }}</span>
        </template>
      </el-table-column>
    </el-table>
  </div>
...
const load = async () => {
  const res = await admin.companies({ status: status.value || undefined, page: 1, size: 50 })
...
const audit = async (row, result) => {
  ...
  if (result === 'REJECTED') {
    try {
      const r = await ElMessageBox.prompt('请填写驳回理由（企业可见）', '驳回企业')
      reason = r.value
    } catch { return }
  }
  try {
    await admin.auditCompany(row.id, { result, reason })
...
<style scoped>
.reason { color: #909399; font-size: 12px; }
</style>
